...
仓库特意跟踪两份结构化 `review/*.pending.json`，因为运行时与测试依赖它们。`inbox` 默认仍被忽略；唯一例外是
`s20260717-114847-312497-2076b1a9`，它是经过隐私遮挡的剑之雨普通闪回归夹具，CI 用它验证真实画面识别。
该夹具仍为 `pending`，不能据此自动批准正式卡牌定义。
...
真机检查时，在游戏中打开剑之雨普通闪详情，运行“识别当前卡牌闪光（只读）”。当前应看到：卡牌和
`common_flash_draw_1` 识别成功、生成 `CardObservation`、但“策略可用”为“否”。整个过程不得点击游戏或
保存截图。
